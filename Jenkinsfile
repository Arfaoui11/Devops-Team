...
pipeline {
    ...
    stages {
      ...
      /*  stage("Run the container with ansible"){
       ...
       stage("nexus deploy"){
              steps {
                  sh 'cd serveur && chmod 777 /var/lib/jenkins/workspace/FullStack-DevOps/serveur/target/classes/ && mvn deploy'
                     }
         }
      ...
      stage("Sonar Quality Check"){
		steps{
		    script{
		     ...
		     sh 'cd serveur && chmod 777 /var/lib/jenkins/workspace/FullStack-DevOps/serveur/target/classes/ && mvn sonar:sonar'
	    	}
